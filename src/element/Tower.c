...
int upgrateTower(Tower* p_courant, Joueur* joueur) {
	...
	if(p_courant != NULL) {
		...
		if(joueur != NULL) {
			...
			if(strcmp("H", p_courant->type_tower) == 0) {
				...
				if(joueur->money >= (p_courant->lvl * 50)) {
					joueur->money -= (p_courant->lvl * 50);
					p_courant->power += 5;
			...
			//Si c'est une tour mitrailette 
				...
				if(joueur->money >= (p_courant->lvl * 100)) {
					joueur->money -= (p_courant->lvl * 100);
					p_courant->power += 2;
					if(p_courant->rate - 1 > 0)
						p_courant->rate -= 1;
					p_courant->range += 3;
			...
			//Si c'est une tour laser 
				...
				if(joueur->money >= (p_courant->lvl * 200)) {
					joueur->money -= (p_courant->lvl * 200);
					p_courant->power += 4;
					if(p_courant->rate - 1 > 0)
						p_courant->rate -= 1;
					p_courant->range += 2;
			...
			//Si c'est une tour rocket 
				...
				if(joueur->money >= (p_courant->lvl * 150)) {
					joueur->money -= (p_courant->lvl * 150);
					p_courant->power += 5;
			...
			fprintf(stderr, "Erreur : cette joueur n'existe pas\n");
			return 0;
		}
...
int upgradePowerT(Tower* p_courant, Joueur* joueur) {
	...
	if(p_courant != NULL) {
		...
		if(joueur != NULL) {
			...
			//Si c'est une tour hybride
			if(strcmp("H", p_courant->type_tower) == 0) {
				if(joueur->money >= (p_courant->lvl * 20))
					joueur->money -= (p_courant->lvl * 20);
			}
			//Si c'est une tour mitrailette 
			else if(strcmp("M", p_courant->type_tower) == 0) {
				if(joueur->money >= (p_courant->lvl * 30))
					joueur->money -= (p_courant->lvl * 30);
			}
			//Si c'est une tour rocket 
			else if(strcmp("R", p_courant->type_tower) == 0) {
				if(joueur->money >= (p_courant->lvl * 50))
			...
			//Si c'est une tour laser 
				...
				if(joueur->money >= (p_courant->lvl * 70))
					joueur->money -= (p_courant->lvl * 70);
			}
		}
		else {
			fprintf(stderr, "Erreur : cette joueur n'existe pas\n");
...
int upgradeRateT(Tower* p_courant, Joueur* joueur) {
		...
		if(joueur != NULL) {
			if(p_courant->rate - 1 > 0) {
				...
				//Si c'est une tour rocket
					...
					if(joueur->money >= (p_courant->lvl * 60))
						joueur->money -= (p_courant->lvl * 60);
				}
				//Si c'est une tour laser 
				else if(strcmp("L", p_courant->type_tower) == 0) {
					if(joueur->money >= (p_courant->lvl * 70))
...
int upgradeRangeT(Tower* p_courant, Joueur* joueur) {
